...
class SearchPane(wx.Panel):
    ...
    def get_word_matches(self, word, options, recursive=False):
        ...
        if not options["CaseSensitive"]:
            ...
            for case in cases:
                if case in index:
                    matches.extend([bytearray(index[case][i:i + 3])
                                    for i in xrange(0, len(index[case]), 3)])
        elif word in index:
            matches.extend([bytearray(index[word][i:i + 3])
                            for i in xrange(0, len(index[word]), 3)])
