...
class NotesPage(wx.Panel):
    def __init__(self, parent, tab):
        ...
        self.toolbar.AddTool(self.ID_SHOW_TOPICS_PANE, "",
                             self._frame.get_bitmap("show-topics-pane"),
                             _("Show Topics Pane"), wx.ITEM_CHECK)
        ...
        self.toolbar.Bind(wx.EVT_MENU, self.OnShowTopicsPane,
                          id=self.ID_SHOW_TOPICS_PANE)
        ...
        self.toolbar.AddTool(wx.ID_JUSTIFY_LEFT, "",
                             self._frame.get_bitmap("align-left"),
        ...
        self.toolbar.AddTool(wx.ID_JUSTIFY_RIGHT, "",
                             self._frame.get_bitmap("align-right"),
        ...
        self.toolbar.AddTool(ID_INCREASE_INDENT, "",
                             self._frame.get_bitmap("increase-indent"),
                             _("Increase Indent"))
    ...
    def OnAlignLeft(self, event):
        if self.editor.HasFocus():
            self.toolbar.ToggleTool(wx.ID_JUSTIFY_LEFT, not self.editor.
                                    IsSelectionAligned(wx.TEXT_ALIGNMENT_LEFT))
            self.toolbar.Refresh(False)
        self.editor.ApplyAlignmentToSelection(wx.TEXT_ALIGNMENT_LEFT)

    def OnAlignCenter(self, event):
        if self.editor.HasFocus():
            self.toolbar.ToggleTool(wx.ID_JUSTIFY_CENTER, not self.editor.
                                    IsSelectionAligned(wx.
    ...
    def OnAlignRight(self, event):
        if self.editor.HasFocus():
            self.toolbar.ToggleTool(wx.ID_JUSTIFY_RIGHT, not self.editor.
                                    IsSelectionAligned(wx.
    ...
    def OnNumbering(self, event):
        ...
        if not self.editor.HasSelection():
            pos = self.editor.XYToPosition(0, self.editor.
                                           PositionToXY(self.editor.
                                                        GetInsertionPoint())
                                           [1])
    ...
    def OnBullets(self, event):
        if not self.editor.HasSelection():
            start = self.editor.XYToPosition(0, self.editor.
                                             PositionToXY(self.editor.
                                                          GetInsertionPoint())
                                             [1])
