...
class ToolBar(aui.AuiToolBar):
    def __init__(self, parent):
        ...
        self.verse_entry = wx.ComboBox(self, choices=parent._app.config.ReadList("VerseHistory"),
                                       size=(150, -1), style=wx.TE_PROCESS_ENTER)
        if self.autocomp_books:
        ...
        if '__WXGTK__' not in wx.PlatformInfo:
            self.chapterctrl = wx.SpinCtrl(self, value=str(parent.reference[1]), size=(60, -1),
                                           min=1, max=BOOK_LENGTHS[parent.reference[0] - 1])
        else:
            self.chapterctrl = wx.SpinCtrl(self, value=str(parent.reference[1]), min=1,
                                           max=BOOK_LENGTHS[parent.reference[0] - 1])
    ...
    def OnHistoryItem(self, event):
        book, chapter, verse = refalize(self._parent.verse_history[event.GetId() -
                                                                   wx.ID_HIGHEST - 1])
    ...
    def OnBack(self, event):
        if not event.IsDropDownClicked():
            ...
            for i in range(self._parent.history_item - 1, -1, -1):
                menu.Append(wx.ID_HIGHEST + i + 1, self._parent.verse_history[i])
                self.Bind(wx.EVT_MENU, self.OnHistoryItem, id=wx.ID_HIGHEST + i + 1)
    ...
    def OnForward(self, event):
        if not event.IsDropDownClicked():
            ...
            menu = wx.Menu()
            for i in range(self._parent.history_item + 1, len(self._parent.verse_history)):
                menu.Append(wx.ID_HIGHEST + i + 1, self._parent.verse_history[i])
                self.Bind(wx.EVT_MENU, self.OnHistoryItem, id=wx.ID_HIGHEST + i + 1)
    ...
    def OnChapter(self, event):
        self._parent.load_chapter(self._parent.reference[0], self.chapterctrl.GetValue())
